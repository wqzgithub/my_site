# created by 2021/3/15 23:41
#forms.py 是一个公共的文件，它包含了相关的form的类
...
class RegForm(forms.Form):
    ...
    password = forms.CharField(label='密码',max_length=16,min_length=6,
                               widget=forms.PasswordInput(attrs={'class':'form-control','placeholder':'请输入密码'}))
    password_again= forms.CharField(label='再输入一次密码',max_length=16,min_length=6,
...
class ChangeNicknameForm(forms.Form):
    ...
    nickname_new = forms.CharField(
        label='新的呢称',
        max_length=20,
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': '请输入新的呢称'})
    )
...
class ChangePasswordForm(forms.Form):
    ...
    new_password_again = forms.CharField(label='请再次输入新密码',max_length=16,
    ...
    def clean_old_password(self):
        #验证旧密码（两种方法，1.使用登陆验证auth.authenticate，2.check_password）
...
class ForgotPasswordForm(forms.Form):
    email = forms.EmailField(label='邮箱',
                               widget=forms.EmailInput(attrs={'class':'form-control','placeholder':'请输入已绑定的邮箱'}))
    verification_code = forms.CharField(label='验证码',required=False,
                widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': '点击“发送验证码”,发送到邮箱'})
    )
    new_password = forms.CharField(label='新密码',max_length=16,
    ...
    def clean_email(self):
        ...
        if not User.objects.filter(email=email).exists():
            raise forms.ValidationError('该邮箱未绑定')
        return email
